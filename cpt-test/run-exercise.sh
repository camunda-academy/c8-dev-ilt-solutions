...
lang_start() {
  ...
  case "${lang}" in
    ...
    java)
      ( cd "${WORKTREE}/java" && \
        CAMUNDA_REST_ADDRESS="${REST}" CAMUNDA_GRPC_ADDRESS="${GRPC}" \
        CAMUNDA_CLIENT_MODE=self-managed CAMUNDA_CLIENT_AUTH_METHOD=none \
        mvn -q -B -DskipTests -DmainClass=io.camunda.training.CamundaApplicationCpt compile exec:java ) >"${logfile}" 2>&1 &
      ;;
    java-spring)
